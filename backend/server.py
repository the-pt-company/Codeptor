...
async def sync_remote_jobs(category: str = "software-dev"):
    """Fetch jobs from Remotive API and sync to MongoDB."""
    ...
    try:
        ...
        for job in jobs_data:
            ...
            # Use publication_date if available, else now
        ...
        # Store sync time in a special collection
        await db.sync_logs.update_one(
            {"type": "jobs", "category": category},
            {"$set": {"last_sync": datetime.now(timezone.utc).isoformat()}},
            upsert=True
        )
        logger.info(f"Successfully synced {len(jobs_data)} jobs for {category}")
    except Exception as e:
...
async def get_jobs(
    ...
    days: Optional[int] = Query(None, description="Only show jobs from the last X days"),
    search: Optional[str] = Query(None, description="Search by title, company, or tags")
):
    """Fetch jobs from database, triggering sync if necessary."""
    # Check if we need to sync
    sync_log = await db.sync_logs.find_one({"type": "jobs", "category": category})
    needs_sync = True
    last_sync = None

    if sync_log:
        last_sync = datetime.fromisoformat(sync_log["last_sync"])
        # Sync if older than 6 hours
        if datetime.now(timezone.utc) - last_sync < timedelta(hours=6):
            needs_sync = False
    
    if needs_sync:
        # For simplicity in this demo, we sync synchronously if needed
        # In a real production app, this would be a background task
        await sync_remote_jobs(category)
        # Refresh sync log
        sync_log = await db.sync_logs.find_one({"type": "jobs", "category": category})
        if sync_log:
            last_sync = datetime.fromisoformat(sync_log["last_sync"])
    ...
    if days:
        ...
        query["publication_date"] = {"$gte": cutoff.isoformat()}
    ...
    # Fetch jobs sorted by publication_date DESC
    cursor = db.jobs.find(query).sort("publication_date", -1).limit(50)
    ...
    response_jobs = []
    for j in jobs:
        pub_date = datetime.fromisoformat(j["publication_date"])
        if pub_date.tzinfo is None:
            pub_date = pub_date.replace(tzinfo=timezone.utc)
        # A job is "new" if posted within last 48 hours
        is_new = (now - pub_date) < timedelta(hours=48)
